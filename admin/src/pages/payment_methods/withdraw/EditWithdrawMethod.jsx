...
const EditWithdrawMethod = () => {
  ...
  const fetchWithdrawMethod = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${base_url}/api/admin/withdraw-methods/single/${id}`);
      ...
      setFormData({
        ...
        rate: data.rate || "",
        withdrawInstruction: data.withdrawInstruction || "",
      ...
      toast.error("Failed to load withdrawal method data");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(
        `${base_url}/api/admin/withdraw-methods/${id}`,
        form_data,
      ...
      Swal.fire({
        ...
        navigate('/dashboard/all-withdraw-methods');
      });
    } catch (err) {
      console.error("Error:", err);
      toast.error(err.response?.data?.message || "Failed to update withdrawal method");
    }
  };
          ...
          <main className={`transition-all duration-300 flex-1 p-6 overflow-y-auto h-[90vh] ${
            ...
            <div className="flex justify-center items-center h-full">
              <div className="text-center">
                ...
                <p className="mt-4 text-gray-600">Loading withdrawal method...</p>
              </div>
            </div>
          </main>
        </div>
          ...
          <div className="p-6">
            <form onSubmit={handleSubmit} className="bg-white rounded-xl p-6 border border-orange-200">
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h1 className="text-2xl font-bold text-gray-900">Edit Withdrawal Method</h1>
                  <p className="text-gray-600 mt-1">Update {formData.gatewayName} details</p>
                </div>
                <button
                  type="button"
                  onClick={() => navigate('/dashboard/all-withdraw-methods')}
                  className="px-4 py-2 border border-orange-300 rounded-lg text-gray-700 hover:bg-orange-50 transition-colors cursor-pointer"
                >
                  Back to List
                </button>
              </div>
              ...
              <div className="mb-8">
                ...
                <div className="relative w-40 h-40 border-2 border-dashed border-orange-300 rounded-lg flex items-center justify-center bg-orange-50 transition-colors">
              ...
              {/* YouTube Link */}
              <div className="mb-6">
                <label className="block text-sm font-medium mb-2 text-gray-900">YouTube Video Link</label>
                <input
                  type="url"
                  name="youtubeLink"
              ...
              <div className="flex gap-4">
                <button
                  type="submit"
                  className="flex-1 bg-orange-600 hover:bg-orange-700 text-white py-4 rounded-lg font-[400] text-lg transition-colors shadow-lg hover:shadow-xl cursor-pointer"
                >
                ...
                <button
                  type="button"
                  onClick={() => navigate('/dashboard/all-withdraw-methods')}
                  className="px-8 bg-gray-500 hover:bg-gray-600 text-white py-4 rounded-lg font-[400] text-lg transition-colors shadow-lg hover:shadow-xl cursor-pointer"
                >
                  Cancel
                </button>
              </div>
