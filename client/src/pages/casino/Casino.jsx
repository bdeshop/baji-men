...
const CasinoContent = () => {
  ...
  const fetchUserFavorites = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Fetch favorite counts for games
        const counts = {};
  ...
  const checkFavoriteStatus = async (gameId) => {
    ...
    try {
      const token = localStorage.getItem('usertoken');
      const response = await axios.get(`${base_url}/api/user/favorites/check/${gameId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      if (response.data.success) {
        setFavoriteCounts(prev => ({
          ...prev,
          [gameId]: response.data.data.favoriteCount
        }));
  ...
  const handleFavoriteGameClick = (game) => {
    ...
    if (fullGame) {
      ...
      navigate(`/game/${game.gameApiID || game.gameId}?provider=${encodeURIComponent(game.provider || '')}&category=casino`);
    }
  };
            ...
            <div className='flex justify-center md:justify-between items-center gap-2 sm:gap-4 w-full mb-4 sm:mb-6'>
              <div className="w-full sm:w-auto relative" ref={categoryRef}>
                ...
                {showCategoryDropdown && (
                  <div className="absolute top-full left-0 text-xs sm:text-sm right-0 bg-[#222] border border-[#333] rounded-lg shadow-lg z-[10000] mt-1 overflow-hidden">
                      ...
                      return (
                        ...
                        >
                          <div className="casino-game-image-container relative overflow-hidden w-full">
                            ...
                            >
                              {isFavorited ? (
                                <IoHeart className="w-4 h-4 sm:w-5 sm:h-5" />
                              ) : (
